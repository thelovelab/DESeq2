...
which differs from \Rclass{SummarizedExperiment} in having associated design 
...
\Rclass{DESeqSummarizedExperiment} from a \Rclass{SummarizedExperiment} \Robject{se}.

<<constructDSE>>=
library("DESeq2")
dseGR <- DESeqSummarizedExperiment(se = se,
                                   design = ~ patient + treatment)
colData(dseGR)$treatment <- factor(colData(dseGR)$treatment,
...
you can use the function \Rfunction{DESeqSummarizedExperimentFromMatrix}.  
...
dse <- DESeqSummarizedExperimentFromMatrix(countData = countData,
                                           colData = colData,
                                           design = ~ condition)
colData(dse)$condition <- factor(colData(dse)$condition,
...
function \Rfunction{DESeqSummarizedExperimentFromHTSeqCount}.  For an 
...
dseHTSeq <- DESeqSummarizedExperimentFromHTSeqCount(sampleTable = sampleTable,
                                                    directory = directory,
                                                    design= ~ condition)
colData(dseHTSeq)$condition <- factor(colData(dseHTSeq)$condition,
                                      levels=c("untreated","treated"))
dseHTSeq
...
and results will be most easily interpretable.  The reason is that the
function \Rfunction{model.matrix} is used by the \Rpackage{DESeq2} 
...
the \Rpackage{pasilla} is provided in the vignette of the data package \Rpackage{pasilla}.
...
last in the design formula).  Extracting results of other variables 
is discussed in section \ref{sec:multifactor}.
...
the function \Rfunction{mcols} called on the results object.
...
been combined into \Robject{conditiontreated}.  For a particular gene, a 
$\log_2$ fold change of $-1$ for \Robject{conditiontreated} here means that 
...
argument \Rcode{name}, which is a combination of the variable and the level 
in the numerator of the fold change.  In addition, there might be minor 
changes made by the \Rfunction{DataFrame} function on column names, for 
instance \texttt{single-read} to \texttt{single.read}. The function 
...
where the count $K_{ij}$ of reads for gene $i$, sample $j$ are modeled using
a negative binomial distribution with a sample-specific size factor $s_j$, a 
fitted mean $\mu_{ij}$ and a gene-specific dispersion parameter $\alpha_i$.  
The coefficients $\beta_i$ give the $\log_2$ fold changes for gene $i$ for 
each column of the model matrix $X$. Dispersions are estimated using a 
Cox-Reid adjusted profile likelihood, as first implemented for RNA-Seq data 
in \Rpackage{edgeR} \cite{CR,edgeR_GLM}.  For further details on dispersion 
estimation and inference, please see the manual pages for the functions 
\Rfunction{DESeq} and \Rfunction{estimateDispersions}.
...
The chi-squared test substitutes \Rfunction{nbinomWaldTest} with 
\Rfunction{nbinomChisqTest} in the last step above.  In this case, the
user provides the full formula (the formula stored in \Rcode{design(dse)}),
...
of parameters in the two models. The Wald test and the chi-squared test
...
from the gene-wise estimates towards the fitted estimates. Depending on 
the sample size, number of covariates, and variance present in the data,
...
\caption{The dispersion estimate plot shows the gene-wise estimates (black),
  ...
  estimates used in testing (blue). The genes with low counts have dispersions 
  which are moved closer to the fitted line than the genes with high counts.}
\label{dispFit}
\end{figure}
...
case the two fit types appear to produce similar curves.
...
to use the geometric mean of gene-wise dispersion estimates to benefit 
from information sharing across genes.
...
\Rfunction{estimateDispersionsGeneEst}. The first function, 
\Rfunction{estimateDispersionsGeneEst}, stores the gene-wise estimates
in the metadata column \Robject{dispGeneEst}. The final function, 
\Rfunction{estimateDispersionsGeneEst}, uses this column and the
...
Information on all of the row-wise calculated values are stored
in the original object.  These are accessible via \Rfunction{mcols}. 
Descriptions are accessible by two calls to \Rfunction{mcols}.
...
and $\log_2$ fold changes of ``B'' vs ``Control''. We are working on an
implementation of contrasts, which would allow the user to compare
the coefficients of ``A'' against ``B''.

\section{Sample-/gene-specific normalization factors}

In some experiments, there might be gene-specific dependencies
...
the model from Section \ref{sec:glm} becomes:
...
are typically on the scale of the predictors (i.e. the log scale for 
