...
level of this variable). Extracting results of other variables 
...
instance \texttt{-} to \texttt{.}. The function
...
ddsGeoMean <- estimateDispersions(dds, fitType="geoMean")
plotDispEsts(ddsGeoMean)
@
...
useForMedian <- mcols(dds)$dispGeneEst > 1e-7
medianDisp <- median(mcols(dds)$dispGeneEst[useForMedian],na.rm=TRUE)
mcols(dds)$dispFit <- rep(medianDisp,nrow(dds))
