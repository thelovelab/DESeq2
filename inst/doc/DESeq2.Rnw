...
\item Why are some p-values set to \texttt{NA}?
  ...
  \item If a row contains a sample with outlying Cook's distance
    (see Section \ref{sec:cooks}), 
    then the p-value and adjusted p-value are set to \texttt{NA}.
  \item If a row is filtered by automatic independent
    filtering (see Section \ref{sec:autoFilt}),
    then the adjusted p-value is set to \texttt{NA}.
  \end{enumerate}
  
\item How do I use the variance stabilized / rlog transformed data for
