...
\incfig{DESeq2-MA}{.5\textwidth}{MA-plot.}{
  This plot shows the $\log_2$ fold changes from the treatment over
  the mean of normalized counts, i.e. the average of counts normalized by
  size factor. The \Biocpkg{DESeq2} package incorporates a prior on $\log_2$
...
\subsubsection{More information on results columns}
...
The results columns for certain genes can be set to \Robject{NA} 
for the following reasons:
  ...
  \item If a row contains a sample with an extreme count
    ...
    These outlier counts are detected by Cook's distance and customization
...
RNA-Seq data can sometimes contain very large counts which can be 
described as outliers (i.e. produced by rare technical artifacts, 
and not related to the biological properties of the sample).
\Biocpkg{DESeq2} calculates, for every gene and for every sample,
a diagnostic test for outliers called Cook's distance.
Cook's distance is a measure of how much a single sample 
is influencing the fitted coefficients for a gene, 
where large values might indicate a count outlier. 
\Biocpkg{DESeq2} automatically flags genes with Cook's distance
above a cutoff, setting the p-value and adjusted p-value to \Robject{NA}.

The default cutoff depends on the sample size and number of parameters
to be estimated: the $.75$ quantile of the $F(p,m-p)$ distribution 
...
The functionality can be disabled by setting \Robject{cooksCutoff} to \Robject{FALSE}
...
With many degrees of freedom (many more samples than number of parameters to 
be estimated), it might be undesirable to remove entire genes from the analysis
which contain a count outlier. An alternate strategy is to replace the 
outlier counts with the trimmed mean over all samples, adjusted by the
size factor for that sample. Replacing the value with the trimmed mean 
will not lead to false positives, as it replaces
...
sample. After an initial fit has been performed, the following code 
can be used to replace count outliers by the trimmed mean, 
and then a new fit can be performed.

<<calculateTrimmedMeans>>=
ddsClean <- dds
p <- ncol(attr(ddsClean,"modelMatrix"))
m <- ncol(ddsClean)
cooksCutoff <- qf(.75, p, m - p)
idx <- which(assays(ddsClean)[["cooks"]] > cooksCutoff)
trimBaseMean <- apply(counts(ddsClean,normalized=TRUE),1,mean,trim=.2)
@ 
...
nullCounts <- as.integer(outer(trimBaseMean,
                               sizeFactors(ddsClean), "*"))
counts(ddsClean)[idx] <- nullCounts[idx]
@ 
...
\subsection{Why are some p-values set to \texttt{NA}?}
  ...
  At various steps in the \Biocpkg{DESeq2} pipeline, certain rows
  will have p-values set to \texttt{NA}:
