...
\subsubsection{\Rclass{SummarizedExperiment} input}

In the \Biocpkg{DESeq2} package, in order to simplify the preparation
of a count matrix, we attempt a closer integration with the core \Bioconductor{}
package \Biocpkg{GenomicRanges}. This should facilitate preparation steps
...
4 human cell cultures \cite{Haglund2012Evidence}. We also subset to the 
samples taken 48 hours after treatment.
...
se <- se[,colData(se)$time == "48h"]
@

The class used by \Biocpkg{DESeq2} is \Rclass{DESeqDataSet},
...
resCtrst
mcols(resCtrst)
table(resCtrst$padj < .1)
plot(attr(resCtrst,"filterNumRej"))
@ 
...
we only need to run the function \Rfunction{nbinomLRT}:
