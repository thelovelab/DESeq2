...
the outlier value with the value predicted by the null hypothesis. It can be implemented
as follows.
...
replaces count outliers by the trimmed mean and performs a new fit.
...
@ 
\fixme{I think it would be good to wrap the above two chunks into a simple convenience function.}
