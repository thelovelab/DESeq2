...
By default, the two functions, \Rfunction{rlogTransformation} and 
\Rfunction{varianceStabilizingTransformation}, will use a design formula 
which includes only an intercept, then re-estimate the mean-dispersion relationship using
\Rfunction{estimateDispersions}. This ensures that the data transformation remains 
