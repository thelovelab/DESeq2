# test for equivalence of DESeq2 estimates
# with those found using IRLS code and 
# using optim
test_betaFitting <- function() {
  m <- 10
  set.seed(1)
  y <- rpois(m,20)
  sf <- rep(1,m)
  ...
  checkEqualsNumeric(betaDESeq, betaIRLS, tolerance=1e-6)
  checkEqualsNumeric(betaDESeq, betaOptim, tolerance=1e-6)
}
