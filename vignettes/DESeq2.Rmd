...
*lfcShrink*. Shrinkage estimation is discussed more in a 
[later section](#altshrink).

Here we provide the `dds` object and the name or number of the
coefficient we want to moderate (where the number refers to the order
of the coefficient as it appears in `resultsNames(dds)`).
It is also possible to specify a `contrast`, instead of `coef`, which
works the same as the `contrast` argument of the *results* function.
...
**Note:** if the shrinkage estimator `ashr` is used in published
...
Here we extend the [discussion of shrinkage estimators](#shrinkage).
...
| method:  | `normal`^1^  | `apeglm` | `ashr`^2^ |
|---|:-:|:-:|:-:|
| Good for ranking by LFC | ✓ | ✓ | ✓ |
| Shrinkage adapts to scale of observed LFCs | ✓ | ✓ | ✓ |
| Preserves "very large" LFC (doesn't over-shrink) |  | ✓ | ✓ |
| Explicitly designed to compute *s-values* [@Stephens2016] |  | ✓ | ✓ |
| Allows use of `coef` | ✓ | ✓ | ✓ |
| Allows use of `lfcThreshold` | ✓ | ✓ |  |
| Allows use of `contrast` | ✓ |  | ✓ |
| Can be used on designs with interaction terms |  | ✓ | ✓ |

**References:** 1. @Love2014; 2. @Stephens2016

Beginning with the first two rows, all shrinkage methods provided by
...
help to preserve what we've called "very large" LFC, and can be used
...
estimated rate of *false sign* among the set of genes with s-value
less than or equal than a given gene's s-value, and @Stephens2016
points out they are analogous to the *q*-value of @Storey2003. The
s-value has a desirable property relative to the adjusted
...
setting a threshold means that the s-values will represent aggregate
probabilities of FSOS events, "false sign or small", where the LFC is
either the wrong sign or less than the threshold distance
from 0. Finally, `normal` and `ashr` can be used with arbitrary specified
...
datasets, which mostly come from a recent manuscript [@Berge2018].

* One can use
  the
  [zinbwave](https://bioconductor.org/packages/release/bioc/html/zinbwave.html) and
  [zingeR](https://github.com/statOmics/zingeR) packages to directly
  model the zero inflation of the counts. These packages have been
  developed lately to work in conjunction with DESeq2 and
  edgeR. The most up-to-date example code for running DESeq2 alongside
  `zinbwave` and `zingeR` can be found
  [here](https://github.com/statOmics/zinbwaveZinger/blob/master/zinbwaveSimulations/trapnell_sims_fc2/trapnell_sims_fc2.Rmd).
* In general, it is best to use `test="LRT"` when working with single
  cell data, over the Wald test. This has been observed across
  multiple benchmarks. The LRT performs better with the abundance of
  zeros -- data on the boundary of the parameter space.
* With single cell data, it is recommended to set the
  parameter `minmu=1e-6`, which can be specified when running
  `DESeq`. This again deals with potentially an abundance of zeros,
  and helps to recover power when the small counts may be meaningful.
