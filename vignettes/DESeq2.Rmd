...
# shrunken estimates of log fold change
resultsNames(dds)
...
that implements the method of *Independent Hypothesis Weighting* [@Ignatiadis2016].
...
argument of `lfcShrink`.  For now, these new estimators only replace
the log2 fold change and standard error columns in the results table,
however, in the next release of DESeq2, we plan to pass along
additional inference quantities such as false sign rates. The options
for `type` are:
...
**Note:** if the shrunken estimates from `type="ashr"` are used in published
...
```{r priorInfo}
priorInfo(resLFC)
str(priorInfo(resApe))
str(priorInfo(resAsh))
```
