...
\begin{itemize}
\item
  \href{http://www.cs.cmu.edu/~ckingsf/software/sailfish/}{Sailfish} \cite{Patro2014Sailfish}
\item \href{http://combine-lab.github.io/salmon/}{Salmon} \cite{Patro2015Salmon}
\item
  \href{https://pachterlab.github.io/kallisto/about.html}{kallisto} \cite{Bray2015Near}
\item \href{http://deweylab.github.io/RSEM/}{RSEM} \cite{Li2011RSEM}
\end{itemize}
...
in gene length across samples (e.g. from differential isoform usage),
(ii) some of these methods (\textit{Sailfish, Salmon, kallisto}) 
are substantially faster and require less memory
and disk usage compared to alignment-based methods, and
(iii) it is possible to avoid discarding those fragments that can
align to multiple genes with homologous sequence.
...
\subsection{If I have multiple groups, should I run them all together or split into pairs of groups?}
...
plot(c(10^rnorm(1000, 3, 2),2e3,5e3), 
     c(rnorm(1000, 0, .2), -8.5, 7.5),
     ylim=c(-10,10), log="x", cex=.4,
     xlab="mean of normalized counts", 
     ylab="log2 fold change")
@ 

\incfig{figure/overShrink-1}{.5\textwidth}{foo bar.}{
  foo bar. note the y-axis is from -10 to 10 on the log2 scale.
}


There could be experiments in which a few genes have
...
e.g. technical replicate where the only difference across libraries 
is the concentration of a few spiked-in genes.
If ``overshrinking'' very large log fold changes is a concern,
it can be assessed by running \Rfunction{results} with \Rcode{addMLE=TRUE}
and then comparing \Rfunction{plotMA} with \Rcode{MLE=TRUE} and \Rcode{MLE=FALSE}.
If the above description seems to fit,
it is better to turn of the log fold change prior by
