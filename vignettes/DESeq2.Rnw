...
An alternative filtering package \Biocpkg{IHW} is now available,
which implements the method of \textit{Independent Hypothesis Weighting} \cite{Ignatiadis2015}.
\textit{IHW} is from the same research group that created \deseqtwo{}.
Here we show the use of \textit{IHW} filtering for \deseqtwo{} results.
