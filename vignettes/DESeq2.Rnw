...
to estimate the $\log_2$ fold changes of the model. A summary of various
design formulae for different experiments is presented later in 
Section~\ref{sec:recDesign}.
...
files packages for the \Biocannopkg{pasilla} package.
...
level as the first element ("base level"), so that the $\log_2$ fold changes
...
\ref{sec:multifactor}. Extracting results for comparisons of different
levels of a factor is discussed in the section on contrasts, 
Section~\ref{sec:contrasts}. All the values calculated by the \deseqtwo{} 
...
For \deseqtwo{}, the function \Rfunction{plotMA} shows the $\log_2$
...
will be colored red if the adjusted p-value is less than 0.1.  
...
The \Rfunction{plotMA} function can also take the object returned by
\Rfunction{results} as its first argument.
...
for either one of the following reasons:
  ...
  \item If a row contains a sample with an extreme count
    ...
    of this outlier filtering is described in Section~\ref{sec:dealingCooks},
  ...
  \item If a row is filtered by automatic independent filtering,
    ...
    Description and customization of independent filtering is described 
...
The data in the \Biocannopkg{pasilla}
package have a condition of interest (the column \Robject{condition}), as well as
the type of sequencing which was performed (the column \Robject{type}), as we can 
see below:
...
variable of interest, we put it at the end of the formula. Then we can re-run 
\Rfunction{DESeq}.

<<saveSingleFactor,echo=FALSE>>=
ddsSingleFactor <- dds
@
...
<<multiTypeResults>>=
resultsNames(dds)
resType <- results(dds, name="type_single.read_vs_paired.end")
head(resType)
@

If the condition of interest contains 3 or more levels, the \Robject{contrast}
argument of \Rfunction{results} should be used to specify which $\log_2$ 
fold changes and p-values should be returned. This is described in detail
in Section~\ref{sec:contrasts}. The \Robject{contrast} argument
can also be used to return the same results as using the \Robject{names} 
argument, but provides more flexibility.

%---------------------------------------------------
\subsection{Recommended design formulae for various experiments} \label{sec:recDesign}
%---------------------------------------------------

\begin{center}
  \begin{table}[!h] \footnotesize
    \begin{tabular}{ p{2in} | p{1.5in} | p{3in}}
    Differences of interest & Design & Notes \\ \hline \hline
    $\ge 3$ level factor 'condition': compare levels against another &
    \Robject{\textasciitilde condition}, or \newline
    \Robject{\textasciitilde group + condition} etc. & 
    \Robject{results(dds, contrast = c("condition", "C", "B"))},
    see \ref{sec:contrasts} \\ \hline
    
    $\ge 3$ level factor 'condition': compare significance of all levels &
    \Robject{\textasciitilde  condition}, or \newline
    \Robject{\textasciitilde group + condition} etc. & 
    \Robject{DESeq(dds, test="LRT", reduced = \textasciitilde 1)} or 
    \Robject{DESeq(dds, test="LRT", reduced = \textasciitilde group)},
    see \ref{sec:LRT} \\ \hline
    
    $2$ level factor 'condition' but 'group' has $\ge 3$ levels &
    \Robject{\textasciitilde group + condition} & 
    default \Robject{DESeq} call, then use \Robject{contrast} instead of \Robject{name}  \\ \hline
    
    Interactions between 'group' and 'treatment' &
    \Robject{\textasciitilde group + treatment + group:treatment} & 
    default \Robject{DESeq} call, 
    \Robject{resultsNames(dds)} to see available interactions and
    \Robject{results(dds,name="group1.treatmentA")}
     \\ \hline
    
    Time series: changes due to treatment after time 0 &
    \Robject{\textasciitilde  time + treatment + time:treatment} &
    default \Robject{DESeq} call for examining individual 
    time-treatment interactions;
    or a likelihood ratio test for any differences
    after time 0, see \ref{sec:LRT} \\ \hline
    
    Dual-assay, e.g.: changes in ratio of RNA-Seq
    to CLIP-Seq reads across treatment &
    \Robject{\textasciitilde  assay + treatment + assay:treatment} & 
    default \Robject{DESeq} call \\
    \end{tabular}
  \end{table}
\end{center}
...
possible differences: $\log_2$ fold change of B vs A, of C vs A,
and of C vs B (the other three possible pairs will simply have
$-1 \times$ the $\log_2$ fold changes of these three). 

In order to fit models with ``shrunken'' $\log_2$ fold changes 
to experimental designs with factors with three or more levels,
\deseqtwo{} uses "expanded model matrices", described further in 
Section~\ref{sec:expanded}. This ensures that the model results 
are independent of the choice of base level. The expanded model matrices 
include a coefficient for each level of the factors in addition to an intercept. 
The \Robject{contrast} argument, as shown below, is then used rather 
than the \Robject{name} argument of the \Rfunction{results} function.

Here we show how to perform contrasrts using the parathyroid dataset 
...
<<paraSetup>>=
# test only the top 1000 genes
ddsCtrst <- ddsPara[1:1000, colData(ddsPara)$time == "48h"]
...
The contrast argument can be used also for designs with factors 
with two levels, in which case it will give same results using
the \Robject{name} argument.
...
by default, genes that are affected by such outliers are set aside by \deseqtwo{}, or if there are sufficient samples, outlier counts are replaced for model fitting. These two behaviors are described below.
...
The function \Rfunction{replaceOutliers} replaces the counts in \Robject{counts(dds)}, saving the original counts as \Robject{originalCounts} in \Robject{assays(dds)}. 
...
<<replaceCooksOutlier>>=
ddsReplace <- replaceOutliers(ddsSingleFactor,minReplicates=3)
@ 
...
\subsection{Tests of $\log_2$ fold change above or below a threshold}
...
of the following four values, where $\beta$ is the $\log_2$ fold change
...
\fixme{Fix \Rfunction{plotMA} so that the warnings are avoided.}

<<lfcThresh>>=
par(mfrow=c(2,2),mar=c(2,2,1,1))
yl <- c(-2.5,2.5)

plotMA(dds, ylim=yl, lfcThreshold=.5, altHypothesis="greaterAbs")
abline(h=c(-.5,.5),col="dodgerblue",lwd=2)

plotMA(ddsNoPrior, ylim=yl, lfcThreshold=.5, altHypothesis="lessAbs")
abline(h=c(-.5,.5),col="dodgerblue",lwd=2)

plotMA(dds, ylim=yl, lfcThreshold=.5, altHypothesis="greater")
abline(h=.5,col="dodgerblue",lwd=2)

plotMA(dds, ylim=yl, lfcThreshold=.5, altHypothesis="less")
abline(h=-.5,col="dodgerblue",lwd=2)
@ 

\incfig{figure/lfcThresh}{.5\textwidth}{MA-plots of tests of $\log_2$ fold
...
%--------------------------------------------------
\section{Theory behind \deseqtwo{}}
%--------------------------------------------------
...
column of the model matrix $X$. Dispersions are estimated using
expected mean values from the maximum likelihood estimate 
of $\log_2$ fold changes, and optimizing the Cox-Reid
adjusted profile likelihood, as first implemented for RNA-Seq data in
\Biocpkg{edgeR} \cite{CR,edgeR_GLM}. The steps performed by the 
\Rfunction{DESeq} function are documented in its manual page; 
briefly, they are:
...
``expanded model matrices'' if there are factors in the design formula
which contain three or more levels, in order to produce
$\log_2$ fold changes which are independent of the choice of base level.
...
The prior variance for the $\log_2$ fold changes is 
...
argument of the \Rfunction{results} function should be used rather than
the \Robject{name} argument to generate contrasts of the levels against
each other.
...
\subsection{Can I perform a \Rfunction{DESeq} analysis without replicates?}

If a \Rclass{DESeqDataSet} is provided with an experimental design without replicates,
a warning message is printed, and the samples are treated as replicates
