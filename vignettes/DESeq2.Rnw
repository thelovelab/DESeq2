...
again used to extract test results of $\log_2$ fold changes of interest.
...
``expanded model matrices'' with the $\log_2$ fold change prior, 
in order to produce $\log_2$ fold change estimates and test 
...
Expanded model matrices are not used without the $\log_2$ fold change 
