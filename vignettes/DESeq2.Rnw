...
% library('knitr'); rm(list=ls()); knit('DESeq2.Rnw'); options(prompt='> ')
...
%  Note that another vignette, ``Beginner's guide to using the DESeq2 package'', covers
%  similar material, but at a slower pace, which might be useful if you are analyzing
%  RNA-Seq or high-throughput sequencing data in R for the first time.
  For an exposition of the statistical method, please see our paper \cite{Love:2014:biorxiv}.
