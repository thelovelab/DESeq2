...
We save the single factor analysis as a separate object, so that we can rerun
the analysis on the \Robject{dds} object.
...
<<multiTypeResults>>=
resultsNames(dds)
resType <- results(dds, contrast=c("type","single.read","paired.end"))
head(resType)
@
...
\begin{center}
  \begin{table}[!h] \footnotesize
    ...
    Compare factor levels against another &
    \ldots & 
    use the contrast argument of \Rfunction{results},
    see \ref{sec:contrasts} \\ \hline
    
    Test the significance of all levels of 'condition' &
    \ldots & 
    use the likelihood ratio test, removing 'condition' in the reduced formula,
    see \ref{sec:LRT} \\ \hline
       
    Interactions between 'group' and 'condition' &
    \Robject{\textasciitilde group + condition + group:condition} & 
    \Robject{resultsNames(dds)} to see available interactions and
    \Robject{results(dds,name="group2.conditionB")}
     \\ \hline
    
    Time series: changes due to condition after time 0 &
    \Robject{\textasciitilde  time + condition + time:condition} &
    default \Robject{DESeq} call for examining individual 
    time-condition interactions;
    or a likelihood ratio test to test for all differences
    after time 0, see \ref{sec:LRT} \\ \hline
    
    Dual-assay, e.g.: changes in ratio of RNA-Seq
    to CLIP-Seq reads across condition &
    \Robject{\textasciitilde  assay + condition + assay:condition} & 
...
ddsLocal <- estimateDispersions(dds[1:1000,], fitType="local")
@
...
ddsMean <- estimateDispersions(dds[1:1000,], fitType="mean")
@
...
ddsMed <- estimateDispersionsGeneEst(dds[1:1000,])
