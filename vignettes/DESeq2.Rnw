...
\textbf{Plain text.} A plain-text file of the results can be exported using the 
...
For $\sim 20$ samples, \Rfunction{rlog} may take 30 seconds, 
...
system.time( vsd <- varianceStabilizingTransformation(dds, blind=FALSE) )
system.time( vsd.fast <- vst(dds, blind=FALSE) )
head(assay(rld), 3)
@
