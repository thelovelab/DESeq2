...
\author{Michael Love$^{1}$, Simon Anders$^{2,3}$, Wolfgang Huber$^{3}$ \\[1em] 
...
\begin{abstract}
  ...
  typical workflows.  An RNA-seq workflow on the Bioconductor website:
  \url{http://www.bioconductor.org/help/workflows/rnaseqGene/} covers
  similar material to this vignette but at a slower pace, including
  the generation of count matrices from FASTQ files.
...
These steps imply you have a \Rclass{RangedSummarizedExperiment} object
...
It is important to provide the correct input for \deseqtwo{}'s
statistical model \cite{Love2014} to hold, as only the actual 
counts allow assessing the measurement precision correctly. The \deseqtwo{}
...
While it is not necessary to pre-filter before running the \deseqtwo{}
functions, there are two reasons which making pre-filtering useful:
by removing rows in which there are no reads or nearly no reads,
we (1) reduce the memory size of the data object and 
(2) increase the speed of the transformation
and testing functions within \deseqtwo{}. Here we perform a minimal
filtering for rows that have only 0 or 1 read. Note that more strict
filtering to increase power is automatically applied via independent filtering
on the mean of \textit{normalized} counts, which will be discussed in
...
differences in counts which are explanable by the experimental design,
...
as unwanted ``noise'', and shrinks the tranformed values towards each
other. By setting \Robject{blind} to \Robject{FALSE}, the dispersions
...
line is used in the transformation.  So setting \Robject{blind} to
\Robject{FALSE} is still mostly unbiased by the information about the
samples.
...
in fact a much simpler approach is possible. We will explain this approach
first. If the comparisons of interest are, for example, the effect
of a condition for different sets of samples, a simpler approach is to:

\begin{enumerate}
\item combine the factors of interest into a single factor, e.g.
  \Robject{group}, with all combinations of the original factors
...
in that it creates set-specific condition effects which
can be extracted with \Rfunction{results}.
If we have two factors \Robject{set} (with values 1 and 2) 
and \Robject{condition} (with values A and B), and we want to test 
for the condition effects specifically for each set, we could use the
following approach: 
...
dds <- DESeq(dds)
resultsNames(dds)
# the condition effect in set #1 would be
results(dds, contrast=c("group", "1B", "1A"))
@

Now we will continue to explain the use of interactions in order to
test for differences in condition effects across sets. We will use as
an example, condition effects across three genotypes (X, Y, and Z).
For a simple diagram of how interactions might look across genotypes,
see Figure \ref{fig:inter}. 
...
\begin{figure}
\centering
\includegraphics[width=.45\textwidth]{figure/interFig-1}
\includegraphics[width=.45\textwidth]{figure/interFig2-1}
\caption{
  \textbf{Interactions.} 
  write text here.}
\label{fig:inter}
...
The two points about interactions which are hardest for users are:

\begin{enumerate}
  \item that the interaction is the difference between reference group effect and the
effect in another group, and therefore the effect in that group is the
sum of the \textit{main effect} and the \textit{interaction effect}
(see \Rcode{?results}).
  \item that the interaction is not necessary, see code above.
\end{enumerate}
...
\textbf{Note on many outlier:} if there are very many outliers 
...
The automatic outlier filtering/replacement is useful in situations which the number
of outliers is limited. When there are thousands, it might make more sense to
turn off the outlier filtering/replacement
...
\begin{itemize}
  \item For the calculation of the beta prior variance, instead of
    ...
    counts. This change occurred in version 1.6 (October 2014).
  \item For designs with interaction terms, the solution described in
    ...
    contrasts was too confusing.  This change occurred in version 1.10
    (October 2015).
\end{itemize}
