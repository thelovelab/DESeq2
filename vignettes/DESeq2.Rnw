...
\begin{abstract}
  ...
  The \deseqtwo{} worklow on the Bioconductor website, covers similar
  material but at a slower pace, including the generation of count
  tables from FASTQ files.
...
Further examples of the use of contrasts can be found in
...
are equal to zero. This is conceptually similar to an analysis of variance (ANOVA)
calculation for linear regression, except the case of the Negative
...
the user needs to provide a reduced formula, e.g. one which does not
contain the variable of interest. The full formula is the one
specified by \Robject{design(dds)}.  The degrees of freedom for the
test is obtained from the difference between the number of parameters
in the two models.
