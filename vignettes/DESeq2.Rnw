...
against the base level. See \ref{sec:contrasts} for examples on how 
to compare factor levels to other levels than the base level.
...
\begin{center}
  \begin{table}[!h] \footnotesize
    ...
    default \Robject{DESeq} call  \\ \hline
...
although we suggest 7 or more replicates in order to consider replacement
for large Cook's distances.

<<replaceCooksOutlier>>=
ddsClean <- replaceOutliersWithTrimmedMean(dds,minReplicates=3)
@ 
...
\Biocpkg{DESeq2} relies on the negative binomial distribution to make
...
regardless of the trend over the continuous covarites. 
