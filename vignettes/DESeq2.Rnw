...
library("DESeq2")
ddsSE <- DESeqDataSet(se = se, design = ~ patient + treatment)
ddsSE$treatment <- factor(ddsSE$treatment,
                            levels=c("Control","DPN","OHT"))
ddsSE
@

An analysis of this dataset is shown in the ``Beginner's guide'' vignette.
...
In order to speed up the total running time of the vignette, 
here subset to only those genes which have
total counts over all samples greater than 10 reads. 
We know that these genes
...
columns. See the manual page and the Beginner's vignette for an
example of the use of \Rfunction{collapseReplicates}.
...
which had the smallest $p$ value from the results table created above.

<<plotCounts, dev="pdf", fig.width=4.5, fig.height=5>>=
plotCounts(dds, gene=rownames(resOrdered)[1], intgroup="condition")
@ 
...
plotting with another library.

<<plotCountsAdv, dev="pdf", fig.width=3.5, fig.height=3.5>>=
d <- plotCounts(dds, gene=rownames(resOrdered)[1], intgroup="condition", 
                returnData=TRUE)
library(ggplot2)
ggplot(d, aes(x=condition, y=count)) + geom_point()
@ 

\begin{figure}
\centering
\includegraphics[width=.45\textwidth]{figure/plotCounts}
\includegraphics[width=.45\textwidth]{figure/plotCountsAdv}
\caption{
  \textbf{Plot of counts} 
...
easily analyzed using model formula including the additional variables. 
...
ddsblind <- estimateDispersionsFit(ddsblind)
...
\incfig{figure/figPCA}{.7\textwidth}{PCA plot.}{
...
Example of the contrast of pairs of three different levels of a factor
is shown in the ``Beginner's guide'' vignette. Further examples can be found in
the examples of the help page for the \Rfunction{results} function.
...
grouping variable like sex or species. Interactions
are specified in R formula using a colon, \Robject{:}, between the two
variables names. Demonstrations of extracting results from an
interaction model are shown in the help page for \Rfunction{results}.
...
\Rfunction{DESeq} replaces outliers by internally calling the \Rfunction{replaceOutliers} function,
...
user needs to provide a reduced formula, e.g. one which does not contain the variable of
interest. The full formula is the one specified by \Robject{design(dds)}.
...
below. We recommend providing a matrix with row means of $1$, 
so that the mean of normalized counts for a gene is close to the mean
of the unnormalized counts.
This can be accomplished by dividing out the current row means.
