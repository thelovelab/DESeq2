...
\author{Michael Love$^{1}$, Simon Anders$^{2}$, Wolfgang Huber$^{2}$ \\[1em] \small{$^{1}$ Department of Biostatistics, Dana Farber Cancer Institute and} \\ \small{Harvard School of Public Health, Boston, US;} \\ \small{$^{2}$ European Molecular Biology Laboratory (EMBL), Heidelberg, Germany} }
...
FAQ at the end of this document for more information about how to
make an informative post.
...
Interactions are specified in R formula using a colon, \Robject{:}, between the two
