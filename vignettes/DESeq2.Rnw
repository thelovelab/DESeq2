...
produced by default will be the expected comparison against the base
...
which is independent to the choice of base level, \deseqtwo{} uses
...
for \Rfunction{results}. 

It is important to remember that interactions are \textit{additional terms} 
(addition in the log scale), beyond the \textit{main effect}
terms. If the \textit{main effect} of a condition on counts is a log2
fold change of 2, and the \textit{interaction term} for the condition
in a particular group Y is a log2 fold change of 0.5, then the log2 fold change for
the condition for group Y is $2 + 0.5 = 2.5$. Testing if the
interation term is equal to zero tests if the condition effect in group Y is
the same for the other group(s). Testing if the sum of the main effect
and the interaction term is equal to zero tests if there is a
condition effect in group Y. This is shown in more detail in the
examples section of the help page for \Rfunction{results}.

Designs with factors with two levels and an interaction term present a
...
model, so there are no base levels.
...
symmetric shrinkage regardless of the base level). 
...
across all groups; while for standard design matrices, the main effect is
only for the base level of the other factors. We explain in more detail below:

Suppose we have an interaction design of \Rcode{$\sim$ group + condition + group:condition}.

\textbf{Expanded model matrices} The following code produces a comparison of B over A 
\textit{across all groups}.
...
table now produces a comparison of B over A only for the \textit{base level} of
\Robject{group}. Let us suppose the base level of group is X, and the
second group is Y. So for standard model matrices, the above results
table gives the condition effect for group X.
...
\textit{interaction effect} for condition in group Y, which tests if
the condition effect is different in group Y than in the other groups.

<<expInter2, eval=FALSE>>=
results(dds, contrast=list("groupY.conditionB","groupY.conditionA"))
@ 

And the following would give the condition effect in group Y (the main
effect and the interaction terms added together):

<<expInter3, eval=FALSE>>=
results(dds, contrast=list(c("conditionB","groupY.conditionB"),
                           c("conditionA","groupY.conditionA")))
@ 

\textbf{Standard model matrices} Here, we have only a single
interaction term, \Robject{groupY.conditionB}.
This term represents the additional effect of condition for
group Y compared to the main effect of condition in group X. By symmetry, it also
represents the additional effect of group in condition B
compared to the main effect of group in condition A. Testing if the
condition effect is different between the two groups is accomplished
...
The effect of condition in group Y is the above interaction term added to
the main effect of condition B vs A in group X:
...
effect'' for both groups, we can use a numeric contrast,
assigning a 1 to the main effect \Robject{conditionB} and a 0.5 to the
interaction effect \Robject{groupY.conditionB}, as they appear in
\Robject{resultsNames(dds)}. This can be understood as taking us
halfway from the condition effect in group X to the condition effect
in group Y:
...
in gene expression at any time point after the base-level time point
