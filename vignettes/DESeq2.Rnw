...
\subsubsection{Transcript abundance estimates summarized to gene-level}

Users can create gene-level count matrices for use with \deseqtwo{}
by importing information from transcript abundance estimators, including the
following methods:
...
We demonstrate the import of transcript abundances
and construction of a gene-level \Rclass{DESeqDataSet} object
