...
It's also useful to examine the counts, normalized to account for
sequencing depth, across the groups. A simple function for making this
plot for one gene at a time is \Rfunction{plotCounts}.

<<plotCounts>>=
plotCounts(dds, gene=rownames(resOrdered)[1], intgroup="condition")
@ 

For customized plotting, the function can be told to just return
a \Rclass{data.frame} for plotting with another library:

<<plotCountsAdv>>=
d <- plotCounts(dds, gene=rownames(resOrdered)[1], intgroup="condition", 
                returnData=TRUE)
library(ggplot2)
ggplot(d, aes(x=condition, y=counts)) + geom_point()
@ 
...
\caption{
  ...
  either made using the simple \Rfunction{plotCounts} function (left)
...
The main effect of OHT treatment over control for all patients 
...
In order to test if the effect for OHT treatment over control
is different for patient 4, we test the interaction term with
the folloing: 
...
This above call returns a results table for the \emph{interaction} of OHT treatment in
patient 4. For the effect due to OHT treatment in patient 4,
we need to add the interaction terms for patient 4 to the main effect terms:

<<interactResGroup>>=
resPat4 <- results(ddsX, contrast =
                   list(c("treatmentOHT","patient4.treatmentOHT"),
                        c("treatmentControl","patient4.treatmentControl")))
head(resPat4, 2)
@ 
