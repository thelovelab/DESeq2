...
\subsection{Tests of $\log_2$ fold change above a threshold}

If small $\log_2$ fold changes are not interesting for the 
experiment being analyzed or the biological question, it is
also possible to provide thresholds for constructing
Wald tests of significance. Two arguments to the \Rfunction{results}
function allow for these threshold-based Wald tests: \Robject{lfcThreshold},
which takes a numeric of the non-negative threshold value, 
and \Robject{altHypothesis} which can take one of four values.
Details on the alternative hypotheses can be found 
in the manual page for \Rfunction{results}.
The four possible values of \Robject{altHypothesis} are demonstrated
in the following code and visually by MA-plots in Figure~\ref{DESeq2-lfcThresh}. 
...
defined by the alternative. The \Robject{"greaterAbs"} test is two-tailed, 
and the \Robject{"lessAbs"} test is the maximum of p-values from
the upper and lower tests.

<<lfcThresh, fig=TRUE>>=
par(mfrow=c(2,2),mar=c(2,2,1,1))
a <- 0.5
yl <- c(-2.5,2.5)
plotMA(dds, ylim=yl, lfcThreshold=a, altHypothesis="greaterAbs")
abline(h=c(-a,a),col="dodgerblue",lwd=2)
plotMA(dds, ylim=yl, lfcThreshold=a, altHypothesis="lessAbs")
abline(h=c(-a,a),col="dodgerblue",lwd=2)
plotMA(dds, ylim=yl, lfcThreshold=a, altHypothesis="greater")
abline(h=a,col="dodgerblue",lwd=2)
plotMA(dds, ylim=yl, lfcThreshold=a, altHypothesis="less")
abline(h=-a,col="dodgerblue",lwd=2)
@ 

\incfig{DESeq2-lfcThresh}{.5\textwidth}{MA-plots of tests of $\log_2$ fold
