...
to estimate the log2 fold changes of the model. A summary of various
...
For \deseqtwo{}, the function \Rfunction{plotMA} shows the log2
...
log2 fold change of $-1$ for \Robject{condition\_treated\_vs\_untreated} means 
...
argument \Robject{contrast}, which takes a character vector of length three:
...
If the variable is continuous or an interaction term, then the results
can be extracted using the \Robject{name} argument to \Rfunction{results},
where the name is one of the characters returned by \Robject{resultsNames(dds)}.
...
rld <- rlogTransformation(ddsblind, blind=FALSE)
...
ddsCtrst <- ddsPara[idx, ddsPara$time == "48h"]
...
including only the type of sequencing, in order to test the log2 fold change
...
of log2 fold changes, and optimizing the Cox-Reid
