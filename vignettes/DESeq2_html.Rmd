...
function of the `r Biocpkg("genefilter")` package, and all of the
...
* `greaterAbs` - $|\beta| > \textrm{lfcThreshold}$ - tests are two-tailed
* `lessAbs` - $|\beta| < \textrm{lfcThreshold}$ - *p* values are the maximum of the upper and lower tests
...
zero-centered Normal prior -- in the software referrred to as a
$\beta$-prior -- hence DESeq2 provides *moderated* log2 fold change
estimates. 
...
* Maximum *a posteriori* estimation of GLM coefficients
...
* For designs with interaction terms, the solution described in
...
likelihood estimates (see the DESeq2 paper).
...
A simulated example of such a dataset is below.
...
An example of a dataset with where the log fold change prior should be
turned off: Here we show a simulated MA-plot, where nearly all of the
log fold changes are falling near the x-axis, with three genes that
have very large log fold changes (note the y-axis is from -10 to 10 on
the log2 scale). This would indicate a dataset where the log fold
change prior would overshrink the large fold changes, and so
`betaPrior=FALSE` should be used. 

```{r overShrink, echo=FALSE}
...
Even more detail: how does DESeq2 avoid overshrinking on typical datasets?
