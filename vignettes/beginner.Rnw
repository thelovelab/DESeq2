...
\begin{abstract}
  ...
  DESeq2 package'', also included with the \deseqtwo{} package, covers more of the advanced details
  and at a faster pace.  For an exposition of the statistical method, please see our paper
  \cite{Love:2014:biorxiv}.
...
The following call can be used for paired-end reads sorted by QNAME, where we
specify no index, because these files do not have a location-based index.
...
While the following call can be used for single-end reads:

<<bamfilesingle, eval=FALSE>>=
library( "Rsamtools" )
...
sampleInfo <- data.frame(run=rev(se$run), 
                         color=rep(c("blue","orange"),length=ncol(se)))
@ 
...
The plot (Fig.\ \ref{MA1}) represents each gene with a dot. The $x$ axis
...
The MA plot (Figure~\ref{MA1}) highlights an important property of RNA-Seq data.
...
# create 10 bins using the quantile function
qs <- c( 0, quantile( res$baseMean[res$baseMean > 0], 0:9/9 ) )
...
\incfig{figure/beginner_ratioSmallP}{.5\textwidth}{Ratio of small p valules}{for groups of genes 
