...
%% \newmdtheoremenv[linecolor=red,backgroundcolor=pink!50]{exer}{Exercise}

%% \usepackage{comment}
%% \includecomment{vignette}
%% \excludecomment{course}

%% <<switch, echo=FALSE>>=
%% # these are used to determine eval
%% vignette <- TRUE
%% course <- FALSE
%% @ 
...
\begin{abstract}
  ...
  expression workflow.  We will start from the FASTQ files, align to
  the reference genome, prepare gene expression values as a count
  table by counting the sequenced fragments, perform differential gene
  expression analysis, and visually explore the results. 
  This vignette is designed for users who are perhaps new to
...
For example, the paired-end RNA-Seq reads for the 
\Biocexptpkg{parathyroidSE} package were aligned using TopHat2 with 8
threads, with the call: 
...
Besides the main count table, which we will use later, the \Biocexptpkg{parathyroidSE} package also
...
\textbf{Note:} make sure that the chromosome names of the genomic
features in the annotation you use are consistent with the chromosome names
of the reference used for read alignment. Otherwise, the scripts might
...
In the previous section, we specified the BAM files which will be used for counting.

We indicate in Bioconductor that these files are BAM files using the \Rfunction{BamFileList}
function. Here we also specify details about how the BAM files should be treated, e.g., 
only process $2000000$ reads at a time. 

<<bamfilepaired>>=
library( "Rsamtools" )
bamLst <- BamFileList( bamFiles, yieldSize=2000000 )
@ 

After these preparations, the actual counting is easy. The function
\Rfunction{summarizeOverlaps} from the \Biocpkg{GenomicAlignments}
...
class in \deseqtwo{} is the \Rclass{DESeqDataSet}, which is built on
top of the \Rclass{SummarizedExperiment} class.  One main differences
is that the \Robject{assay} slot is instead accessed using the
\Rfunction{count} accessor, and the values in this matrix must be
non-negative integers.

A second difference is that the \Rclass{DESeqDataSet} has an
associated ``design formula''.  The design is specified at the
beginning of the analysis, as it will inform many of the \deseqtwo{}
functions how to treat the samples in the analysis (one exception is
the size factor estimation, i.\,e., the adjustment for differing
library sizes, which does not depend on the design formula).  The
design formula tells which variables in the column metadata table
(\Robject{colData}) specify the experimental design and how these
...
\deseqtwo{} uses the same formula notation as, for instance, the
\Rfunction{lm} function of base R. If the question of interest is
whether a fold change due to treatment is different across groups, for
example across patients, ``interaction terms'' can be included using
models such as \Rfunction{\lowtilde{} patient + treatment +
patient:treatment}.  More complex designs such as these are covered in
the other \deseqtwo{} vignette.
...
Make sure that the order of rows in your column data table matches the order of columns in the \Robject{assay} data slot.

Once we have our fully annotated \Rclass{SummarizedExperiment} object, we can construct a 
\Rclass{DESeqDataSet} object from it, which will then form the staring point of the actual \Biocpkg{DESeq2}
package, described in the following sections. Here, we use the \Rclass{SummarizedExperiment} object
...
you have your data not in the form of a \Rclass{SummarizedExperiment} object, but
either as a simple matrix of count values or as output files from the \emph{htseq-count}
...
Below we demonstrate using \Rfunction{DESeqDataSetFromMatrix}.

\subsubsection{Starting from count tables}

In this section, we will show how to build an \Rclass{DESeqDataSet}
...
In this count table, each row represents an Ensembl gene, each column
...
We also have metadata on each of the samples (the ``columns'' of the
count table).
...
In order to speed up the vignette, here subset to only those genes which have
total counts over all samples greater than 10 reads. We know that these genes
have counts which are too small to detect significant differences for
typical RNA-Seq datasets. However, this step is not necessary
for the \deseqtwo{} method, as the independent filtering step will
remove these genes before multiple test correction.

<<subsetRows>>=
rs <- rowSums(counts(dds))
table(cut(rs, c(-1,0,10,Inf)))
dds <- dds[rs > 10,]
@ 
...
We load the \Biocpkg{AnnotationDbi} package and the annotation package \Biocannopkg{org.Hs.eg.db}:
...
(``Hs''), organized as an \Biocpkg{AnnotationDbi} package (``db''), using
...
package is currently a bit cumbersome, so we provide the following
...
convert to, the fourth is the \Rclass{AnnotationDb} object to
use. Finally, the last argument specifies what to do if one source ID
maps to several target IDs: should the function return an NA or simply
the first of the multiple IDs?

To convert the Ensembl IDs in the rownames of \Robject{res} to gene
symbols and add them as a new column, we use:
...
an empirical Bayesian prior in the form of a \emph{ridge penalty}, this is done such
...
<<beginner_samplePCA, dev="pdf", fig.width=4.3, fig.height=3>>=
